...
async def QuantGetInfo(
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        async with session.request("post", base_path + path, params=query_params, json=data.dict()) as inital_response:
            ...
            return ProductV1quantInfoResponse(**response) if response is not None else ProductV1quantInfoResponse()
